...
public class EventHandler {
        ...
        if(event.getState()== Blocks.LEAVES.getDefaultState().withProperty(BlockOldLeaf.VARIANT, BlockPlanks.EnumType.OAK)){
            for( int i=0;i<event.getDrops().size();i++){
                if(event.getDrops().get(i).getItem()==Items.APPLE){
                    if(Config.disableApples){event.getDrops().remove(i);}
                }
            }
            if(rand.nextInt(100)< Config.AcornDropChance){event.getDrops().add(new ItemStack(Content.itemAcorns, 1));
    ...
    public static void onEntityTick(LivingEvent.LivingUpdateEvent event) {
        if(!Config.acornsTemptPigs){return;}
    ...
    {
        if(!Config.acornsBreedPigs){return;}
